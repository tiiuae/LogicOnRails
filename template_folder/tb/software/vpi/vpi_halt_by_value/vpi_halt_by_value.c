...
static int cb_value_change(p_cb_data cb_data) {
    ...
    vpi_printf("%sHALT BY VALUE%s : %s[time: %0llu ps]%s %s -> %d\n",
...
static int track_calltf(char *userdata) {
    ...
    vpi_printf("%sVPI%s: registered %halt-by-value%s request on %s, init value %llx\n", 
...
void (*vlog_startup_routines[])(void) = { register_track_halt, 0 };
